...
function searchBooks(searchVal) {
...
function addBook(book) {
    ...
    const newBook = _formatNewBook(volumeInfo)
...
function _formatNewBook(volumeInfo) {
    return {
        ...
        listPrice: {
            ...
            currencyCode: Math.random() > 0.5 ? 'ILS' : 'USD',
...
function query(filterBy = null) {
    ...
    const filterBooks = _getFilteredBooks(books, filterBy)
...
function getPriceFormatted(price, currencyCode) {
    let localeForamt;
    switch (currencyCode) {
        case 'EUR':
            localeForamt = 'fr-FR'
...
function getPageCountProp(pageCount) {
    let pageCountProp;
    if (pageCount > 500) pageCountProp = 'Long Reading'
    else if (pageCount > 200) pageCountProp = 'Decent Reading'
    else if (pageCount < 100) pageCountProp = 'Light Reading'
    else pageCountProp = '';
    return pageCountProp
}

function getPulishedDateProp(publishedDate) {
    let publishedDateProp;
    const currYear = new Date().getFullYear()
    const diff = currYear - publishedDate

    if (diff < 1) publishedDateProp = 'New!'
    else if (diff > 10) publishedDateProp = 'Veteran Book'
    else publishedDateProp = '';
    return publishedDateProp
}

function getPriceProp(price) {
    let priceProp;

    if (price > 150) priceProp = 'price price-red';
    else if (price < 20) priceProp = 'price price-green';
    else priceProp = 'price'
    return priceProp
}
